...
initialState = GameState {
  ...
  food = initFood cantHuevos ((snake initialState)++(obstaculos initialState)) ,
  direction = (1, 0),
  gameOver = False,
  score=0
}
...
initFood cantHuevos snake = (randomItems (0,0) cantHuevos snake)
...
updateFood food (x,y) n invalids=
  if( length food == 1) then randomItems (x,y) n invalids
...
update _ gameState@(GameState { obstaculos = obstaculos ,snake = (x, y):xs, direction = (dx, dy), food = food, gameOver = False ,score=score}) =
    if x' < -10 || x' > 10 || y' < -10 || y' > 10 || (x', y') `elem` xs || (x', y') `elem` obstaculos
        ...
        then gameState { snake = (x', y'):  (x,y):xs, food = updateFood food (x', y') cantHuevos (((x',y'):(x,y):xs)++food++obstaculos) ,score=score+1}
    else 
...
render GameState {obstaculos=obstaculos, snake = (x,y):xs, food = food, gameOver = False ,score=score} =
  pictures [renderCeldas,renderObstaculos obstaculos,renderSnake xs , renderFood food, renderFoodVerde food, renderHead (x,y) ,renderScore score]
  where
